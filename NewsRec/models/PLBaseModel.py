...
class SequencePoolingLayer(nn.Module):
    ...
    def forward(self, seq_value_len_list):
        ...
        if self.mode == 'mean':
            ...
            hist = torch.div(hist, user_behavior_length.type(torch.float32) + self.eps.to(uiseq_embed_list.device))
...
class PLBaseModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 在 pl 中不需要 to(device)
        self.reg_loss = torch.zeros((1,),device=self.device)
        self.aux_loss = torch.zeros((1,),device=self.device)
    ...
    def test_step(self, batch, batch_idx):
        ...
        Args:
            ...
            batch_idx (int): Integer displaying index of this batch
        """
        test_loss = self._step(batch, batch_idx)
        return test_loss
