...
# 加载点击数据，这里分成线上和线下，如果是为了获取线上提交结果应该讲测试集中的点击数据合并到总的数据中
...
def gen_data_set(data, negsample=0):
    ...
    item_ids_set = data['click_article_id'].unique()
    ...
    for reviewerID, hist in tqdm(data.groupby('user_id')):
        ...
        if negsample > 0:
            candidate_set = list(item_ids_set - set(pos_list))  # 用户没看过的文章里面选择负样本
...
def gen_data_set_plus(data, item_emb, item_index_2_rawid, negsample=0):
    ...
    for reviewerID, hist in tqdm(data.groupby('user_id')):
        ...
        if len(pos_list) == 1:
            train_set.append(
                (reviewerID, [pos_list[0]], pos_list[0], item_emb[item_index_2_rawid[pos_list[0]]], 1, len(pos_list)))
            test_set.append(
                (reviewerID, [pos_list[0]], pos_list[0], item_emb[item_index_2_rawid[pos_list[0]]], 1, len(pos_list)))
        ...
        for i in range(1, len(pos_list)):  # 对从2开始的每个位置都根据前面的预测
            ...
            if i != len(pos_list) - 1:
                train_set.append((reviewerID, hist[::-1], pos_list[i], item_emb[item_index_2_rawid[pos_list[i]]], 1,
                                  len(hist)))  # 正样本 [user_id, his_item, pos_item,pos_item_emb, label, len(his_item)]
                for negi in range(negsample):
                    train_set.append((reviewerID, hist[::-1], neg_list[i * negsample + negi],
                                      item_emb[item_index_2_rawid[neg_list[i * negsample + negi]]], 0,
                                      len(hist)))  # 负样本 [user_id, his_item, neg_item,neg_item_emb, label, len(his_item)]
            else:
                # 将最长的那一个序列长度作为测试数据
                test_set.append(
                    (reviewerID, hist[::-1], pos_list[i], item_emb[item_index_2_rawid[pos_list[i]]], 1, len(hist)))
...
def gen_model_input(train_set, seq_max_len):
    ...
    train_model_input = {"user_id": train_uid, "click_article_id": train_iid,
                         'click_article_emb': train_click_article_emb
        , "hist_article_id": train_seq_pad, "hist_len": train_hist_len}

    return train_model_input, train_label

# 时序信息加位置编码不行 不止使用用户点击序列 使用item点击序列
def youtubednn_u2i_dict(data, item_emb_df=None, topk=20, resume=False, max_epochs=1, negsample=0):
    ...
    raw_user_id = data[["user_id"]].drop_duplicates('user_id')
    raw_item_id = data[["click_article_id"]].drop_duplicates('click_article_id')
    print('len(raw_user_id)', len(raw_user_id))
    # 类别编码
    ...
    continue_user_id = data[["user_id"]].drop_duplicates('user_id').reset_index()  # user_id已经被LabelEncoder改变了
    continue_item_id = data[["click_article_id"]].drop_duplicates('click_article_id').reset_index()
    user_index_2_rawid = dict(zip(continue_user_id['user_id'], raw_user_id['user_id']))
    item_index_2_rawid = dict(zip(continue_item_id['click_article_id'], raw_item_id['click_article_id']))
    print('user_index_2_rawid', list(user_index_2_rawid.items())[:10])
    print('item_index_2_rawid', list(item_index_2_rawid.items())[:10])

    # 划分训练和测试集
    # 由于深度学习需要的数据量通常都是非常大的，所以为了保证召回的效果，往往会通过滑窗的形式扩充训练样本

    train_set, test_set = gen_data_set(data,negsample=negsample)
    ...
    #                         'click_article_emb': all_item_emb}  #
    #
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume_sample', default=False, action='store_true')

    args = parser.parse_args()
    max_epochs = args.max_epochs
    resume_sample = args.resume_sample
    ...
    if not metric_recall:
        user_multi_recall_dict['youtubednn_recall'] = youtubednn_u2i_dict(all_click_df, item_emb_df=None, topk=20,
                                                                          resume=False, max_epochs=max_epochs,
                                                                          negsample=negsample)
    else:
        if not resume_sample:
            hist_click_df, last_click_df = get_hist_and_last_click(all_click_df)
            hist_click_df.to_csv('./hist_click_df.csv', index=False)
            last_click_df.to_csv('./last_click_df.csv', index=False)
        else:
            hist_click_df, last_click_df = pd.read_csv('./hist_click_df.csv'),pd.read_csv('./last_click_df.csv')
        # trn_hist_click_df 没有最后点击的item 召回评估时不是所有item都有embedding
        ...
        user_multi_recall_dict['youtubednn_recall'] = youtubednn_u2i_dict(trn_hist_click_df, item_emb_df=None, topk=20,
                                                                          resume=False, max_epochs=max_epochs,
                                                                          negsample=negsample)
        # 召回效果评估
