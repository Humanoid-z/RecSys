...
from sklearn.preprocessing import LabelEncoder
from models.two_tower import YouTubeDNN
...
def gen_model_input(train_set,user_profile,seq_max_len):
    ...
    # train_click_article_emb = []
    # for line in train_set:
    #     print('train_click_article_emb.shape',line[3].shape)
    #     train_click_article_emb.append(line[3])
    # train_click_article_emb = np.array(train_click_article_emb)
...
def youtubednn_u2i_dict(data,item_emb_df, topk=20, resume=False,max_epochs=1,negsample=0):
    ...
    # print('item_profile_', item_profile_)
    # print('item_idx_2_rawid_dict', list(item_idx_2_rawid_dict.items())[:10])
    # exit()
    # 划分训练和测试集
    ...
    model = YouTubeDNN(user_feature_columns, item_feature_columns,
                       user_dnn_hidden_units=(64, embedding_dim),device=device)

    model.fit(train_model_input, train_label, batch_size=256, max_epochs=max_epochs)
