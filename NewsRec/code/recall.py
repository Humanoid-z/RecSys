...
parser.add_argument('--binetwork_w', default=0.8,type=float)
parser.add_argument('--w2v_w', default=0.1,type=float)
parser.add_argument('--hot_w', default=3,type=float)
...
binetwork_w, w2v_w, hot_w= args.binetwork_w,args.w2v_w,args.hot_w
...
if __name__ == '__main__':
    ...
    weights = {'itemcf': 1, 'binetwork': binetwork_w, 'w2v': w2v_w,'hot':hot_w}
    recall_list = []
    recall_dict = {}
    for recall_method in recall_methods:
        ...
        recall_result['sim_score'] = mms(recall_result) #相似度做归一化
    ...
    for user_id, g in tqdm(gg):
        if g['label'].isnull().sum() > 0:   # online
    ...
    log.debug(f'All hitrate: {len(df_useful_recall)/len(recall_final)}')
    ...
    if mode == 'valid':
        ...
        print('binetwork_w, w2v_w, hot_w: ',binetwork_w, w2v_w,hot_w)
...
# 召回合并后指标: 0.38789805752893836, 0.23249706062562872, 0.5095881563912763, 0.24889641376613436, 0.6126484177057976, 0.25613216342453654, 0.6964826866709722, 0.2591487542172082, 0.7193352873962939, 0.2596560315699613
# binetwork_w, w2v_w:  0.8 0.1
# 召回合并后指标: 0.38740126186099655, 0.2329106430191906, 0.5090168413731432, 0.2493853139314963, 0.6082020964777187, 0.256314031951167, 0.6923841224104526, 0.2593170292552219, 0.7156838392369218, 0.25983371056142074
# binetwork_w, w2v_w:  1 0.1

# 召回合并后指标: 0.42242535645089174, 0.2527087783794561, 0.554846241740772, 0.27050010566765936, 0.6631228575686821, 0.2781172594996004, 0.7418898107208505, 0.2809527031441848, 0.7651646877639227, 0.28146719094591416
# 召回合并后指标: 0.4154702170997069, 0.23313709904449334, 0.5563614685279944, 0.25216852294762765, 0.6741268816135924, 0.2604420534910197, 0.7599731730339312, 0.26353537976941177, 0.7805653534701178, 0.26399159744162715 ⭐

# All hitrate: 0.943053665458508    hot recall 50