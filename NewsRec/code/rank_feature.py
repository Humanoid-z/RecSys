...
def func_w2w_last_sim(x):
    ...
    return sim
#计算前后一小时内的点击量
# def get_clicks_in_range(last_click_timestamp, articles_time):
#     # 获取该用户最后点击时间前后一小时的时间范围
#     start_time = last_click_timestamp - 1
#     end_time = last_click_timestamp + 1
#     # 筛选出在这个时间范围内的点击记录
#     time_filtered1 = articles_time[articles_time['click_timestamp'] == start_time]
#     time_filtered2 = articles_time[articles_time['click_timestamp'] == end_time]
#     time_filtered3 = articles_time[articles_time['click_timestamp'] == last_click_timestamp]
    
#     # 返回点击量
#     return len(time_filtered1)+len(time_filtered2)+len(time_filtered3)
def get_clicks_in_range(last_click_hour, article_id, article_hour_counts):
    # 计算该小时及其前后小时的点击量总和
    total = 0
    for h in [last_click_hour - 1, last_click_hour, last_click_hour + 1]:
        total += article_hour_counts.get((article_id, h), 0)
    return total
...
if __name__ == '__main__':
    ...
    ## hot 相关 找到recall的article在最后一次点击的前后一小时的点击量

    last_click_time = df_click.drop_duplicates(subset='user_id', keep='last')[['user_id', 'click_timestamp']].reset_index(drop=True).rename(columns={'click_timestamp': 'user_last_click_time'})
    ...
    articles_time['click_timestamp'] = articles_time['click_timestamp'] / (1000*3600)
    articles_time['click_timestamp'] = articles_time['click_timestamp'].astype('int')
    article_hour_counts = articles_time.groupby(['article_id', 'click_timestamp']).size().to_dict()
    # 合并用户最后一次点击时间与文章点击时间
    df_feature = pd.merge(df_feature, last_click_time, on='user_id', how='left')
    df_feature['user_last_click_time'] = df_feature['user_last_click_time'] / (1000*3600)
    df_feature['user_last_click_time'] = df_feature['user_last_click_time'].astype('int')
 
    df_feature['clicks_in_range'] = df_feature.parallel_apply(
        lambda row: get_clicks_in_range(row['user_last_click_time'], row['article_id'], article_hour_counts), axis=1
    )
    # df_feature['clicks_in_range'] = df_feature.parallel_apply(
    #     lambda row: get_clicks_in_range(row['user_last_click_time'], articles_time[articles_time['article_id']==row['article_id']]), axis=1
    # )
